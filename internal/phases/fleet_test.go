...
func TestFleet_DryRunUsesTheEKSFleetCheckout(t *testing.T) {
	...
	//
	// Hardcoding an absolute path under one developer's home would make the property this
	// test advertises ("commands against paths that exist in the eks-fleet checkout")
	// verifiable on exactly one machine: in CI the fixture is built from the same path
	// strings the phase applies, so it could only ever agree with itself. If
