...
- **`--force-buckets` is two acts, and sometimes zero** — `force_destroy` has no effect until an apply has landed it in state, so injecting it only on the destroy path fails on `BucketNotEmpty`. The flag is applied first, then the teardown runs. Against druid outside development it refuses outright: landing-zone pins the tenant Aurora's `deletion_protection` inside a `map(object)` that no `TF_VAR` can reach without also replacing the leaf's sizing, and continuing would delete the per-tenant deepstorage buckets (no versioning, no expiry, only copy) and *then* fail on `DeleteDBCluster` — segments gone, Aurora standing, sweep halted with the cluster, VPC and NAT gateways still billing.
