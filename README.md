...
| 4 | substrate | `secrets` → `agent-iam` → `managed-monitoring` → `dns` → `model-import` → `cluster-addons` — every AWS resource ArgoCD will consume, built before ArgoCD exists |
...
- **Account-scoped substrate in a cluster-scoped teardown** — `model-import` (opt-in via `agentPlatform.modelImport`, phase 4) provisions the Bedrock Custom Model Import staging bucket and import role for the whole **account and region**; landing-zone's own component header says tearing a cluster down must not destroy a model another cluster is serving. So rackctl applies it and never destroys it — it is excluded from both the substrate teardown and `rackctl destroy`, which print what survives and how to remove it deliberately. A symmetric teardown would also wedge: the bucket is versioned with no `force_destroy`, so once weights are staged the destroy fails `BucketNotEmpty` and halts the reverse loop with the cluster still standing and billing.
